...
#' @param outliersIndivVariance A [`numeric`] vector.
#' @param outliersBernouilliProba A [`numeric`] vector.
...
#' @export
chronoOutliers_Gauss <- function(M, s, outliersIndivVariance, outliersBernouilliProba,
                                 studyPeriodMin, studyPeriodMax, refYear = NULL,
  ...
  if (length(M) != length(outliersIndivVariance))
    stop("Vector of measurements and vector of outliersIndivVariance should have the same length")
  if (length(M) != length(outliersBernouilliProba))
    stop("Vector of measurements and vector of outliersBernouilliProba should have the same length")

  # Conversion in BC/AD format
  if (!is.null(refYear)) M <- refYear - M

  # Bayesian model
  model.file <- system.file(package = "ArchaeoChron", "model", "chronoOutliers_Gauss.txt")

  print('Modeling done with JAGS following')
  jags <- jags.model(
    file = model.file,
    data = list('Nbobs' = length(M), 'M' = M, 's' = s,
                'sigma.delta' = outliersIndivVariance,
                'p' = outliersBernouilliProba,
